export function createRecipeCard(container, recipe, mediaFilename) {
  ...
  container.innerHTML += `<img src="../assets/pictures/${mediaFilename}.jpg" class="recipe-card_picture" alt="${
    recipe.name
  }"></img>
  ...
  <div class="recipe-card_bottom">
    ...
    <p class="recipe-card_description">${recipe.description.length > 170? recipe.description.slice(0, 170)+"...":recipe.description}</p>
  </div>`
}
...
export function createIngredientsList(recipeId, ingredientData) {
  ...
  if (ingredientData.unit == "cuillères à soupe" && ingredientData.quantity == 1) {
    ingredientData.unit = "cuillère à soupe"
  }
  ingredientsContainer.innerHTML += `<li><span class="recipe-card_ingredients-list_ingredient-name">${
